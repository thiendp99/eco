  ...
  persist(
    (set, get) => ({
        ...
        set((state) => {
          ...
          if (existingItem) {
            const items = state.items.map(
              (i) => i.id === product.id ? { ...i, quantity: Math.min(i.quantity + 1, i.stock) } : i
            );
      ...
      updateQuantity: (id, quantity) => {
        ...
        set((state) => {
          const items = state.items.map(
            (item) => item.id === id ? { ...item, quantity: Math.min(quantity, item.stock) } : item
          );
          return { items, ...computeTotals(items) };
        });
      },
...
export { createJSONStorage as c, persist as p, useCartStore };
